...
class WorkoutSessionStore extends ChangeNotifier {
  ...
  void reset() {
    ...
    _session = WorkoutSessionData.mock;
    _completedExercises =
        _session.exercises.map((exercise) => exercise.isCompleted).toList();
  ...
  bool isExerciseCompleted(int exerciseIndex) => _completedExercises[exerciseIndex];
  ...
  WorkoutExercise exerciseAt(int exerciseIndex) => _session.exercises[exerciseIndex];
